...
class ObVecJsonTableTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.client = ObVecJsonTableClient(
            user_id="e5a69db04c5ea54adf324907d4b8f364", admin_id="0", user="jtuser@test"
        )
        self.client2 = ObVecJsonTableClient(
            user_id="11b6dab4c97244fc801797d0e9814074", admin_id="0", user="jtuser@test"
        )
    ...
    def test_create_and_alter_jtable(self):
        ...
        tmp_client = ObVecJsonTableClient(
            user_id="e5a69db04c5ea54adf324907d4b8f364", admin_id="0"
        )
        self.assertEqual(
            sub_dict(tmp_client.jmetadata.meta_cache["t2"], keys_to_check),
            [
                {
                    "jcol_id": 16,
                    "jcol_name": "c1",
                    "jcol_type": "INT",
                    "jcol_nullable": False,
                    "jcol_has_default": True,
                    "jcol_default": "10",
                },
                {
                    "jcol_id": 17,
                    "jcol_name": "c2",
                    "jcol_type": "VARCHAR(30)",
                    "jcol_nullable": True,
                    "jcol_has_default": True,
                    "jcol_default": "'ca'",
                },
                {
                    ...
                    "jcol_type": "VARCHAR",
                    "jcol_nullable": False,
                    "jcol_has_default": False,
                    "jcol_default": None,
                },
                {
                    "jcol_id": 19,
                    "jcol_name": "c4",
                    "jcol_type": "DECIMAL(10,2)",
                    "jcol_nullable": True,
                    "jcol_has_default": False,
                    "jcol_default": None,
                },
        ...
        self.assertEqual(
            sub_dict(self.client.jmetadata.meta_cache["t2"], keys_to_check),
            [
                {
                    "jcol_id": 16,
                    "jcol_name": "c1",
        ...
        self.client.perform_json_table_sql("ALTER TABLE t2 DROP c3")
        self.assertEqual(
            sub_dict(self.client.jmetadata.meta_cache["t2"], keys_to_check),
            [
                {
        ...
        self.assertEqual(
            ...
            [
                ...
                },
                {
                    "jcol_id": 20,
                    "jcol_name": "email",
                    "jcol_type": "VARCHAR(100)",
                    "jcol_nullable": True,
                    "jcol_has_default": True,
                    "jcol_default": "'example@example.com'",
                },
            ],
        )
        ...
        self.client.perform_json_table_sql("ALTER TABLE t2 RENAME TO alter_test")
        self.assertEqual(self.client.jmetadata.meta_cache.get("t2", []), [])
        self.assertEqual(
            sub_dict(self.client.jmetadata.meta_cache["alter_test"], keys_to_check),
            [
                {
                    "jcol_id": 16,
                    "jcol_name": "c1",
    ...
    def test_create_and_alter_jtable_evil(self):
        ...
        self.assertEqual(
            sub_dict(self.client.jmetadata.meta_cache["t1"], keys_to_check),
            [
                {
                    "jcol_id": 16,
                    "jcol_name": "c1",
                ...
                },
                {
                    ...
                    "jcol_nullable": True,
                    "jcol_has_default": True,
                    "jcol_default": "CURRENT_TIMESTAMP()",
                },
            ],
        )
    ...
    def test_dml(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                },
                {
                    ...
                    "jcol_nullable": True,
                    "jcol_has_default": True,
                    "jcol_default": "'2024-12-30T03:35:30'",
                },
            ],
        )
        ...
        res = self.client.perform_json_table_sql("select * from t1")
        self.assertEqual(
            get_all_rows(res),
            [
                (
                    ...
                    "foo",
                    None,
                    datetime.datetime(2024, 12, 30, 3, 35, 30),
                ),
                (
                    None,
                    "world",
        ...
        self.client.perform_json_table_sql("delete from t1 where c1 is NULL")
        res = self.client.perform_json_table_sql("select * from t1")
        self.assertEqual(
            get_all_rows(res),
            [
        ...
        self.client.perform_json_table_sql("select c1, c2, t1.c3 from t1 where c1 > 21")
        self.assertEqual(get_all_rows(res), [])

        self.client.perform_json_table_sql("alter table t1 drop column c3")
        res = self.client.perform_json_table_sql("select * from t1")
        self.assertEqual(
            get_all_rows(res),
            [
    ...
    def test_col_name_conflict(self):
        ...
        res = self.client.perform_json_table_sql("select * from t1 where user_id > 1")
        self.assertEqual(
            get_all_rows(res),
            [
                (2, "bob"),
    ...
    def test_timestamp_datatype(self):
        ...
        self.client.perform_json_table_sql("select * from t1")
    ...
    def test_online_cases(self):
        ...
        logger.info(
            sub_dict(self.client.jmetadata.meta_cache["table_unit_test"], keys_to_check)
        )
        self.assertEqual(
            sub_dict(
                self.client.jmetadata.meta_cache["table_unit_test"], keys_to_check
            ),
            [
                {
                    "jcol_id": 16,
    ...
    def test_user_group(self):
        ...
        logger.info(
            sub_dict(
                self.root_client.jmetadata.meta_cache["table_shared"], keys_to_check
            )
        )
        target_schema = [
            {
        ...
        self.client.refresh_metadata()
        logger.info(
            sub_dict(self.client.jmetadata.meta_cache["table_shared"], keys_to_check)
        )
        self.assertEqual(
            sub_dict(self.client.jmetadata.meta_cache["table_shared"], keys_to_check),
            target_schema,
        ...
        self.client2.refresh_metadata()
        logger.info(
            sub_dict(self.client2.jmetadata.meta_cache["table_shared"], keys_to_check)
        )
        self.assertEqual(
            sub_dict(self.client2.jmetadata.meta_cache["table_shared"], keys_to_check),
            target_schema,
        ...
        res = self.client.perform_json_table_sql("SELECT * FROM `table_shared`")
        self.assertEqual(
            get_all_rows(res),
            [(1, "foo", Decimal("10.00")), (2, "bar", Decimal("20.00"))],
        )
        res = self.client2.perform_json_table_sql("SELECT * FROM `table_shared`")
        self.assertEqual(get_all_rows(res), [(3, "oceanbase", Decimal("100.00"))])

        # multi-users mode
        mu_client = ObVecJsonTableClient(user_id=None, admin_id="0", user="jtuser@test")
        res = mu_client.perform_json_table_sql("SELECT * FROM `table_shared`")
        self.assertEqual(
            get_all_rows(res),
            [
                (1, "foo", Decimal("10.00")),
        ...
        mu_client.perform_json_table_sql("DELETE FROM `table_shared` WHERE c3 = 100.0")
        res = mu_client.perform_json_table_sql("SELECT * FROM `table_shared`")
        self.assertEqual(get_all_rows(res), [(2, "bar", Decimal("20.00"))])
        res = self.client.perform_json_table_sql("SELECT * FROM `table_shared`")
        self.assertEqual(get_all_rows(res), [(2, "bar", Decimal("20.00"))])
        res = self.client2.perform_json_table_sql("SELECT * FROM `table_shared`")
        self.assertEqual(get_all_rows(res), [])
    ...
    def test_select_with_data_id(self):
        ...
        res = self.client.perform_json_table_sql(
            "SELECT * FROM `table_shared`", select_with_data_id=True
        )
        self.assertEqual(
            get_all_rows(res),
            [(1, 1, "foo", Decimal("10.00")), (2, 2, "bar", Decimal("20.00"))],
    ...
    def test_insert_update_delete_with_row_count(self):
        ...
        logger.info(
            sub_dict(
                self.root_client.jmetadata.meta_cache["test_dml_with_row_count"],
                keys_to_check,
            )
        )
        target_schema = [
        ...
        client_without_user_id = ObVecJsonTableClient(
            user_id=None, admin_id="0", user="jtuser@test"
        )
        ...
        row_cnt = client_without_user_id.perform_json_table_sql(
            "DELETE FROM `test_dml_with_row_count` WHERE c1=3", opt_user_id="2"
        )
        self.assertEqual(row_cnt, 1)
        res = client_without_user_id.perform_json_table_sql(
            "SELECT * FROM `test_dml_with_row_count`",
